    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map("<C-h>", function() vim.lsp.buf.signature_help() end, 'Signature [H]elp', { 'n', 'i' })
      ...
      for name, config in pairs(servers) do
        local configLsp = config or {}
        -- This handles overriding only values explicitly passed
        -- by the server configuration above. Useful when disabling
        -- certain features of an LSP (for example, turning off formatting for ts_ls)
        configLsp.capabilities = vim.tbl_deep_extend('force', {}, capabilities, configLsp.capabilities or {})
        vim.lsp.config(name, configLsp)
      end
